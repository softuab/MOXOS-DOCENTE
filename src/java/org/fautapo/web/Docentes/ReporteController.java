...
import org.fautapo.domain.ImprimirLibretaForma2;
...
public class ReporteController {
    ...
    @RequestMapping(value = "/docente/imprimirEvaluacion.fautapo", method = RequestMethod.GET)
    public void imprimirEvaluacion(Model modelo, HttpServletResponse response) throws IOException {
        Clientes cliente = this.GetUsuario();
        if (cliente == null) {
        ...
        if (sIdTipoEvaluacion.equals("1")) {
            ImprimirLibretaForma2 libre = new ImprimirLibretaForma2(libretadocente, parametroslibreta, notaminimaporprograma.getNota_minima(), response);
            libre.CreateLibretaRegular();
        }
        if (sIdTipoEvaluacion.equals("3")) {
            ImprimirLibretaForma2 libretaverano = new ImprimirLibretaForma2(libretadocente, parametroslibreta, 51, response);
            libretaverano.CreateLibretaVerano();
        }
        if (sIdTipoEvaluacion.equals("4")) {
            ImprimirLibretaForma2 libre = new ImprimirLibretaForma2(libretadocente, parametroslibreta, 51, response);
            libre.CreateLibretaMesa();
        }
    }

    private Exception getException(String detalle) {
        return new Exception(detalle);
    }
