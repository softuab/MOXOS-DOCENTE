...
<section class="lockscreen-content">|
    <div class="lock-box"><img class="rounded-circle user-image" src="${simagen}">
        ...
        <form:form name="forma" method="post" cssClass="unlock-form" modelAttribute="model" action="${pageContext.request.contextPath}/rrhh/ListarFuncionesBoleta">
            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
            <form:hidden path="carnet"/>
            <form:hidden path="tipo"/>
            <div class="mb-3">
                <label class="control-label">GESTION</label>
                <form:select path="iDGestion" cssClass="form-control form-select" onchange = "CargarPlanillas()"  >
                    <form:options items="${model.gestion}" itemValue="id" itemLabel="value"/>
                </form:select>
            </div>
            <div class="mb-3">
                <label class="control-label">MES</label>
                <form:select path="idMes" cssClass="form-control form-select"  onchange = "CargarPlanillas()" >
                    <form:options items="${model.mes}" itemValue="id" itemLabel="value"/>
                </form:select>
            </div>
            <div class="mb-3">
                <label class="control-label">TIPO PLANILLA</label>
                <select id="iDPlanilla" name="iDPlanilla" class="form-control form-select"></select>
            </div>
...
<script>
    function CargarPlanillas() {
        var html = '';
        var data = {
            iDGestion: document.getElementById('iDGestion').value,
            idMes: document.getElementById('idMes').value
        }
        Get("${pageContext.request.contextPath}/rrhh/ListarPlanillas",data)
            .then(function (value) {
