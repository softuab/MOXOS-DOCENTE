...
public class BookMark {
    ...
    /**
     * When replacing the label, set it after the label *
     */
    public static final int INSERT_AFTER = 0;
    /**
     * When replacing the label, set it in front of the label *
     */
    public static final int INSERT_BEFORE = 1;
    /**
     * When replacing tags, replace the bookmark with the content *
     */
    public static final int REPLACE = 2;
    /**
     * Part of the constant reference defined in docx *
     */
    ...
    /**
     * Internal label definition class *
     */
    private CTBookmark _ctBookmark = null;
    /**
     * The paragraph where the tag is located *
     */
    private XWPFParagraph _para = null;
    /**
     * The table cell object where the label is located *
     */
    ...
    /**
     * Constructor, used for labels in the table
     *
     * @param ctBookmark
     * @param para
     * @param tableCell
    ...
    public void insertTextAtBookMark(String bookmarkValue, int where) {
        ...
        //According to the type of label, perform different operations
        if (this._isCell) {
            this.handleBookmarkedCells(bookmarkValue, where);
        } else {

            //Ordinary label, create an element directly
    ...
    public void insertHtmlAtBookMark(String bookmarkValue, String value) throws Exception {
        int pos = 0;
        for (IBodyElement bodyElement : _para.getBody().getXWPFDocument().getBodyElements()) {
        ...
        this.handleHtmlBookmarkedCells(bookmarkValue, pos, value);
    }

    public void insertTableAtBookMark(Table bookmarkValue, int where) {
        //Ordinary label, create an element directly
        XWPFTable table = null;
        XmlCursor cursor = this._para.getCTP().newCursor();
        table = this._para.getBody().insertNewTbl(cursor);
        int twipsPerInch = 1440;

        for (int i = 0; i < bookmarkValue.getRow(); i++) {
            XWPFTableRow row = table.getRow(i);
            if (row == null) {
                row = table.createRow();
            }
            for (int j = 0; j < bookmarkValue.getCol(); j++) {
                XWPFTableCell cell = row.getCell(j);
                if (cell == null) {
                    cell = row.createCell();
                }
                CTTblWidth tblWidth = cell.getCTTc().addNewTcPr().addNewTcW();
                tblWidth.setW(BigInteger.valueOf(bookmarkValue.getColumnsSize().get(j)));
                tblWidth.setType(STTblWidth.DXA);
                XWPFParagraph p = cell.getParagraphs().get(0);
                p.setAlignment(bookmarkValue.getData(i, j).getConfig().getAligment());
                XWPFRun r1 = p.createRun();
                r1.setBold(bookmarkValue.getData(i, j).getConfig().isBold());
                r1.setFontSize(bookmarkValue.getData(i, j).getConfig().getSize());
                r1.setFontFamily(bookmarkValue.getData(i, j).getConfig().getFontfamily());
                r1.setText(bookmarkValue.getData(i, j).getData());
            }
        }
        for (int i = 0; i < bookmarkValue.getColSpan().size(); i++) {
            ColSpan value = bookmarkValue.getColSpan().get(i);
            CreateWordTableMerge.mergeCellVertically(table, value.getCol(), value.getFromRow(), value.getToRow());
        }
        for (int i = 0; i < bookmarkValue.getRowSpan().size(); i++) {
            RowSpan value = bookmarkValue.getRowSpan().get(i);
            CreateWordTableMerge.mergeCellHorizontally(table, value.getRow(), value.getFromCol(), value.getToCol());
        }
    }
    ...
    private void handleBookmarkedTableCells(Table bookmarkValue, int where) {
    ...
    private void handleHtmlBookmarkedCells(String bookmarkValue, int position, String name) throws Exception {
        org.jsoup.nodes.Document document = Jsoup.parse(bookmarkValue, "UTF-8");
        String xmlContent = document.html();
        CreateWordWithHTMLaltChunk.addHtml(this._para, position, "chunk" + name, xmlContent);
    }
}
