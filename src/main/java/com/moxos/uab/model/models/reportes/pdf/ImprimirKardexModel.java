...
public class ImprimirKardexModel {
    ...
    public ImprimirKardexModel(KardexPersonal libreta, HttpServletResponse response, String path) {
    ...
    public void Createkardex() throws IOException {
        ...
        try {
            try {
                ...
                HeaderFooterKardexPageEvent event = new HeaderFooterKardexPageEvent(kardexpersona, path);
    ...
    private void crearDatosModalidad(PdfPTable table) {
        ...
        for (DetallePersonaModalidadIngreso modalidad : kardexpersona.getModalidad()) {
    ...
    private void crearDatosIdioma(PdfPTable table) {
        ...
        for (DetalleIdiomaPersonal idioma : kardexpersona.getIdioma()) {
    ...
    private void crearDatosFormacionPregrado(PdfPTable table) {
        ...
        int i = 1;
        List<DetalleFormacionAcademicaPersonal> listformacionacademica = kardexpersona.getFormacionacademica().stream().filter(p -> p.getTipotitulo().equals("PREGRADO")).collect(Collectors.toList());
        for (DetalleFormacionAcademicaPersonal formacionacademica : listformacionacademica) {
    ...
    private void crearDatosPosgrado(Document document, float[] f7) throws DocumentException {
        ...
        for (ListViewItem view : kardexpersona.getPosgrado()) {
            List<DetalleFormacionAcademicaPersonal> listformacionacademica = kardexpersona.getFormacionacademica().stream().filter(p -> p.getId_nivelestudio() == Integer.valueOf(view.getId())).collect(Collectors.toList());
    ...
    private void crearDatosDetallePosgrado(PdfPTable table, List<DetalleFormacionAcademicaPersonal> listformacionacademica) {
        table.setWidthPercentage(100);
        table.setSpacingBefore(0f);
        table.setSpacingAfter(0f);
