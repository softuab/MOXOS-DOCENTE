(function ($) {
    var $preview_cell, tableX = 'tableX', longitud = 0, tableX_class = '.' + tableX, tableX_cell = 'tableX-cell', tableX_disabled = tableX_cell + '-disabled', tableX_cell_class = '.' + tableX_cell,
            active_cell = 'active-cell', active_cell_class = '.' + active_cell, tableX_txtbox = 'tableX-txt-box', tableX_txtbox_class = '.' + tableX_txtbox,
            projected = 'projected', text_area_html = '<input type="number" class="' + tableX_txtbox + '"/>', selected_area = 'selected-area',
            selected_area_class = '.' + selected_area, clicked = false, edit = false, STRING = 'string', ctrl_key_pressed = false,
            ...
            case key_codes.a:
                if (ctrl_key_pressed) {
                    ...
                    if ($(e.target).hasClass('search-input'))
                        return;
                    if ($(e.target).hasClass(tableX_txtbox))
                        return;
                    $active_cell.text('');
                    $active_cell.trigger('dblclick');
                }
    ...
    function copyAll($active_cell) {
        ...
        $selected_area_rows.each(function (index, row) {
            ...
            copied_data_array[index] = $cells.map(function () {
    ...
    function navDown($active_cell, active_cell_index) {
        ...
        var target_tr = $parent_tbody.find(TR).get($parent_tr.index() + 1);
        var $target_cell = $($(target_tr).find(TD).get(active_cell_index));
        var rowindex = $parent_tr.index();
        if (rowindex == (longitud - 1)) {
            $active_cell.trigger('click');
        } else
        {
    ...
    function guardaryvalidar($active_cell)
    {
        var value = $active_cell;
        var token = $("meta[name='_csrf']").attr("content");
        var header = $("meta[name='_csrf_header']").attr("content");
        var request;
        if (value.data("estado") == 0)
        {
            value.text(value.data("nota"));
            swal("Aviso", "Esta intentando modificar una nota con fecha fuera de plazo comunicarse con DTIC.", "error");
        } else
        {
            if (isNaN(value.text()))
            {
                swal("Aviso", "Introdujo un valor no numerico vuelva a registrar la nota del alumno", "error");
                value.text(value.data("nota"));
            } else
            {
                if (parseInt(value.text()) > 100)
                {
                    swal("Aviso", "El dato : " + value.text() + "  no es un numero valido. Introduzca un numero del 0 al 100.", "error");
                    value.text(value.data("nota"));
                    return false;
                } else
                {
                    var cadena = "id_tipo_nota_s=" + $("#id_tipo_nota_s").val() + "&id_programa=" + $("#id_programa").val() + "&id_tipo_grado=" + $("#id_tipo_grado").val() + "&nro_nota_s=" + value.data("nronota") + "&id_asignacion=" + $("#id_asignacion").val() + "&nota:" + value.data("idestudiante") + "=" + value.text() + "&ubicacion=" + $("#ubicacion").val();
                    $.ajax({
                        type: "get",
                        url: './State.fautapo',
                        success: function (data) {
                            if (data.status === 'on_line') {
                                $.ajaxSetup({
                                ...
                                request = $.ajax({
                                    url: "./RegistrarNotasEstudiantes.fautapo",
                                    method: "POST",
                                    data: cadena
                                });
                                ...
                                if (data.status === 'expirado') {
                                    swal("Oops", "su inicio de sesion ha expirado", "error");
                                } else {
                                    swal("Oops", "se encuentra fuera de linea", "error");
                                }
                            }
                        },
                        error: function (data) {
                            switch (true) {
                                ...
                                case (data.status >= 500 && data.status < 600):
                                    ...
                                    break;
                                case (data.status === 404):
                                    swal("Oops", "el host ha sido capaz de comunicarse con el servidor, pero no existe el recurso que ha sido pedido", "error");
                                    break;
                                case (data.status === 0):
                                    swal("Oops", 'No es posible conectar con el servidor, revisa tu conexión de internet.', "error");
                                    break;
                                default:
